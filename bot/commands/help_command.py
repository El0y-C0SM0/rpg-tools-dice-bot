...
- `**`: Eleva o resuldado ao valor.'''
    ...
    @commands.slash_command(name='help', description='Mostra todos os comandos disponíveis.')
    async def help(self, inter):
        await inter.response.defer()

        embed = disnake.Embed(title="RPGTools", description="Estou aqui para rolar seus dados :)", color=0x00ff00)
        embed.add_field(name="Comandos", value=self.get_commands_description(), inline=False)
        embed.add_field(name="Tipos de sigilo", value=self.get_hidden_types_description(), inline=False)
        embed.add_field(name="Solicitações", value=self.get_solicitacoes_description(), inline=False)
        embed.add_field(name="Operadores", value=self.get_operadores_description(), inline=False)
        embed.set_footer(text="Feito por El0y C0sm0s (https://github.com/El0y-C0SM0).", icon_url="https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png")
